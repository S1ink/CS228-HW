...
public class StoutList<E extends Comparable<? super E>> extends AbstractSequentialList<E> {
	 ...
	 */
	public StoutList(int node_dims) {
		if (node_dims <= 0 || node_dims % 2 != 0) {
			throw new IllegalArgumentException("Node dimensionality is not even!");
		}
		this.node_dims = node_dims;

		this.head = new Node();
		this.tail = new Node();
		this.head.next = this.tail;		// NOTE: static Node.link(a, b) here???
	...
	private class Node {
		 ...
		 */
		public E[] data = (E[]) new Comparable[node_dims];	// Unchecked warning unavoidable.
		...
		{
			E item = data[offset];
			for (int i = offset + 1; i < node_dims; ++i)
			{
			data[i - 1] = data[i];
			}
			data[count - 1] = null;
